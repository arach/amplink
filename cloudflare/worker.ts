...
import { PlexusSession } from "./plexus-session.ts";

export { PlexusSession };
  ...
  async fetch(request: Request, env: CloudflareEnv): Promise<Response> {
    ...
    if (request.method === "POST" && url.pathname === "/start-session") {
      ...
      const userId = resolveUserId(
        request,
        env.PLEXUS_DEFAULT_USER || "anonymous",
        body ?? undefined,
      );
    ...
    if (request.method === "GET" && url.pathname === "/ws") {
      ...
      const durableObjectId = env.PLEXUS_SESSION.idFromName(sessionId);
      const stub = env.PLEXUS_SESSION.get(durableObjectId);
      const durableUrl = new URL("https://plexus-session.internal/connect");
