// AmplinkApp — Entry point for the Amplink iOS client.
//
...
private let bootLogger = Logger(subsystem: "com.amplink.ios", category: "Boot")

@main
struct AmplinkApp: App {
    ...
    init() {
        ...
        bootLogger.notice("Amplink app launching")
    ...
    var body: some Scene {
        WindowGroup {
            ...
            .task {
                ...
                AmplinkLog.voice.info("Preloading Parakeet model at app launch")
                do {
                    try await ParakeetModelManager.shared.downloadAndLoad()
                    AmplinkLog.voice.info("Parakeet ready")
                } catch {
                    AmplinkLog.voice.warning("Parakeet preload failed: \(error.localizedDescription)")
                }
                #endif
            }
