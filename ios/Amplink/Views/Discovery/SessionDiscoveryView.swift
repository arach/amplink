        ...
        Button {
            ...
            HStack(spacing: AmplinkSpacing.md) {
                RoundedRectangle(cornerRadius: 3, style: .continuous)
                    .fill(AmplinkColors.accent.opacity(0.15))
    ...
    private var loadingState: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: AmplinkSpacing.sm) {
                BrailleSpinner()
                Text(scanStatus)
                    .font(AmplinkTypography.code(12, weight: .medium))
                    .foregroundStyle(AmplinkColors.accent)
                    .lineLimit(1)
                Spacer()
            }
            .padding(.horizontal, AmplinkSpacing.lg)
            .padding(.vertical, AmplinkSpacing.md)
            .background(AmplinkColors.accent.opacity(0.06))
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: AmplinkSpacing.sm) {
                RoundedRectangle(cornerRadius: 4, style: .continuous)
                    .fill(AmplinkColors.textMuted.opacity(0.15))
                    .frame(width: 14, height: 14)
                RoundedRectangle(cornerRadius: 4, style: .continuous)
                    .fill(AmplinkColors.textMuted.opacity(0.12))
            ...
            ForEach(0..<3, id: \.self) { idx in
                HStack(spacing: AmplinkSpacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        RoundedRectangle(cornerRadius: 3, style: .continuous)
                            .fill(AmplinkColors.textMuted.opacity(0.08))
...
struct BrailleSpinner: View {
    ...
    var body: some View {
        Text(Self.frames[index])
            .font(AmplinkTypography.code(14, weight: .bold))
            .foregroundStyle(AmplinkColors.accent)
            .onAppear {
