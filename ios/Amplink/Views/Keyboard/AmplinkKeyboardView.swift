// AmplinkKeyboardView — SwiftUI wrapper around AmplinkCompactKeyboard (UIKit).
//
...
// MARK: - AmplinkKeyboardView (SwiftUI API)

struct AmplinkKeyboardView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            if isMinimized {
                ...
                .frame(height: AmplinkCompactKeyboard.preferredHeight)
            }
        }
        .background {
            Color.clear
...
private struct AmplinkKeyboardRepresentable: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> AmplinkKeyboardHostView {
        let host = AmplinkKeyboardHostView()
        bindCallbacks(host)
        return host
    }

    func updateUIView(_ host: AmplinkKeyboardHostView, context: Context) {
        bindCallbacks(host)
    }

    private func bindCallbacks(_ host: AmplinkKeyboardHostView) {
    ...
    private func reportPage(_ host: AmplinkKeyboardHostView) {
        let label: String
...
/// Wraps AmplinkCompactKeyboard with swipe gesture handling
/// and edge gesture exclusion zones.
final class AmplinkKeyboardHostView: UIView, UIGestureRecognizerDelegate {
    let keyboard = AmplinkCompactKeyboard()
...
private struct ToolbarButton: View {
    let icon: String
    let label: String?
    var tint: Color = AmplinkColors.textSecondary
    let action: () -> Void
