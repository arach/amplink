// SessionStore — Observable state container for Amplink sessions.
//
...
final class SessionStore: @unchecked Sendable {
    ...
    /// Append a local user text turn and return its synthetic turn ID.
    @discardableResult
    func appendLocalUserTurn(text: String, sessionId: String) -> String? {
        ...
        appendLocalTurn(userTurn, sessionId: sessionId)
        return turnId
    }

    /// Remove a previously appended local-only turn.
    func removeLocalTurn(turnId: String, sessionId: String) {
        lock.lock()
        ...
        state.turns.remove(at: turnIndex)
    ...
    /// Route a single AmplinkEvent to the appropriate handler.
    private func routeEvent(_ event: AmplinkEvent) {
        switch event {
    ...
    private static func seqDefaultsKey(for bridgeKey: String) -> String {
        "amplink.lastAppliedSeq.\(bridgeKey)"
    }
