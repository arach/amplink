// AmplinkLog — Dual-output logging: os.Logger (Console.app) + in-memory LogStore (in-app viewing).
//
...
enum AmplinkLog {
    enum Level: String, CaseIterable {
        ...
        var color: Color {
            switch self {
            case .debug: AmplinkColors.textMuted
            case .info: AmplinkColors.textSecondary
            case .warning: AmplinkColors.statusStreaming
            case .error: AmplinkColors.statusError
            case .fault: AmplinkColors.statusError
            }
        }
    ...
    // Category loggers
    static let voice = AmplinkLogger(category: "voice")
    static let session = AmplinkLogger(category: "session")
    static let network = AmplinkLogger(category: "network")
    static let ui = AmplinkLogger(category: "ui")
    static let security = AmplinkLogger(category: "security")
}

// MARK: - AmplinkLogger (per-category)

struct AmplinkLogger {
    let category: String
    private let osLogger: Logger

    init(category: String) {
        self.category = category
        self.osLogger = Logger(subsystem: "dev.amplink", category: category)
    }
    ...
    private func log(_ level: AmplinkLog.Level, _ message: String, detail: String?) {
