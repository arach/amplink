// ConnectionManager — WebSocket + Noise encryption + RPC client for the Amplink bridge.
//
...
// AmplinkIdentity from Security/Identity.swift for Keychain-based key/trust storage,
...
/// The bridge's trust record (public key) lives in AmplinkIdentity (Keychain).
struct BridgeConnectionInfo: Codable, Sendable, Equatable {
    var relayURL: String
...
final class ConnectionManager: @unchecked Sendable {
    ...
    /// Relay URL currently associated with the paired bridge.
    var bridgeRelayURL: String? {
        connectionInfo?.relayURL
    }

    /// Relay room currently associated with the paired bridge.
    var bridgeRoomID: String? {
    ...
    private func ensureIdentity() throws -> NoiseKeyPair {
        ...
        let keyPair = try AmplinkIdentity.loadOrCreateIdentity()
    ...
    func connect(qrPayload: QRPayload) async throws {
        ...
        do {
            ...
            try AmplinkIdentity.saveTrustedBridge(publicKey: remoteKey)
    ...
    func reconnect() async {
        ...
        guard AmplinkIdentity.isTrustedBridge(publicKey: remoteKeyData) else {
        ...
        while attempt < Self.maxReconnectAttempts {
            ...
            do {
                ...
                try? AmplinkIdentity.touchTrustedBridge(publicKey: remoteKeyData)
                startMessageLoop()
                setState(.connected)
    ...
    /// Start a reconnect attempt owned by the connection manager so it can be
    /// cancelled cleanly by `disconnect()`.
    func startReconnect() {
    ...
    func clearTrustedBridge() {
        ...
        if let info = connectionInfo {
            ...
            if keyData.count == 32 {
                try? AmplinkIdentity.removeTrustedBridge(publicKey: keyData)
            }
        }
        connectionInfo = nil
        UserDefaults.standard.removeObject(forKey: "amplink.connectionInfo")
    ...
    private static func loadConnectionInfo() -> BridgeConnectionInfo? {
        guard let data = UserDefaults.standard.data(forKey: "amplink.connectionInfo") else {
            return nil
        }
    ...
    private func saveConnectionInfo(_ info: BridgeConnectionInfo) {
        if let data = try? JSONEncoder().encode(info) {
            UserDefaults.standard.set(data, forKey: "amplink.connectionInfo")
        }
    }
