// ConnectionManager — WebSocket + Noise encryption + RPC client for the Plexus bridge.
//
...
// PlexusIdentity from Security/Identity.swift for Keychain-based key/trust storage,
...
/// The bridge's trust record (public key) lives in PlexusIdentity (Keychain).
...
final class ConnectionManager: @unchecked Sendable {
    ...
    private func ensureIdentity() throws -> NoiseKeyPair {
        ...
        let keyPair = try PlexusIdentity.loadOrCreateIdentity()
    ...
    func connect(qrPayload: QRPayload) async throws {
        ...
        do {
            ...
            try PlexusIdentity.saveTrustedBridge(publicKey: remoteKey)
    ...
    func reconnect() async {
        ...
        guard PlexusIdentity.isTrustedBridge(publicKey: remoteKeyData) else {
        ...
        while attempt < Self.maxReconnectAttempts {
            ...
            do {
                ...
                try? PlexusIdentity.touchTrustedBridge(publicKey: remoteKeyData)
                startMessageLoop()
                setState(.connected)
    ...
    func clearTrustedBridge() {
        ...
        if let info = connectionInfo {
            ...
            if keyData.count == 32 {
                try? PlexusIdentity.removeTrustedBridge(publicKey: keyData)
            }
        }
        connectionInfo = nil
        UserDefaults.standard.removeObject(forKey: "plexus.connectionInfo")
    ...
    private static func loadConnectionInfo() -> BridgeConnectionInfo? {
        guard let data = UserDefaults.standard.data(forKey: "plexus.connectionInfo") else {
            return nil
        }
    ...
    private func saveConnectionInfo(_ info: BridgeConnectionInfo) {
        if let data = try? JSONEncoder().encode(info) {
            UserDefaults.standard.set(data, forKey: "plexus.connectionInfo")
        }
    }
